// JALANKAN SEMUA PROSES INI DI TERMINAL / CMD / POWERSHELL
// mkdir gemini-flash-api
// cd gemini-flash-api
// npm init -y
// npm install express dotenv multer @google/genai

// Buat file namanya .env dan di isi seperti dibawah :
//GEMINI_API_KEY=

// PROSES DIBAWAH INI DITULIS DI index.js
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const result = await model(["Analyse this document:", documentPart]);
      res.json({ output: result.text });
    } catch (error) {
      res.status(500).json({ error: error.message });
    } finally {
...
// Buat file namanya .gitignore dengan isi dibawah :
// node_modules
// .env
